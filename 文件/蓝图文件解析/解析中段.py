...
def 解析分拣器姿态(流解析器) -> 分拣器姿态:
    return 分拣器姿态(
        起点=解析全空间姿态(流解析器),
        终点=解析全空间姿态(流解析器),
    )

def 解析建筑(流解析器) -> list[建筑]:
    (建筑数,) = 流解析器.解析("i")
    所有建筑 = []
    for i in range(建筑数):
        num, = 流解析器.解析("i")
