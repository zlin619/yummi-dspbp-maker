...
from 蓝图格式.全息信标额外参数Util import 标记等级 as 信标标记等级, 信息等级 as 信标信息等级
import 日志
...
class 额外参数之全息信标(解析后额外参数):
    ...
    标记等级: 信标标记等级  # 离线/视野范围内/本地星球/本地星系/全星区
    信息等级: 信标信息等级  # 不常显信息/常显图标/常显标题/常显待办事项
    全息图标: 图标  # 图标ID，如 建筑:全息信标
    IP地址: 类型.Int32
    参数类型: str = "全息信标"
    ...
    def 尝试构造(cls, 未解析参数: 额外参数之未解析) -> 额外参数:
        ...
        参数 = 未解析参数.参数
        if len(参数) < 7:
            参数 = list(参数) + [类型.Int32(0)] * (7 - len(参数))
        return cls(
            颜色=类型.Int32(int(参数[0])),
            投影高度_百分之一米=类型.Int32(int(参数[1])),
            投影半径_百分之一米=类型.Int32(int(参数[2])),
            标记等级=信标标记等级(int(参数[3])),
            信息等级=信标信息等级(int(参数[4])),
            全息图标=图标(int(参数[5])),
            IP地址=类型.Int32(int(参数[6])),
        )
    ...
    def 去解析(self) -> 额外参数之未解析:
        参数列表 = [
            ...
            类型.Int32(int(self.标记等级)),
            类型.Int32(int(self.信息等级)),
            类型.Int32(self.全息图标.转int()),
            类型.Int32(self.IP地址),
        ]
