...
class 额外参数之未解析(额外参数):
    ...
    def 尝试解析(self, 目标模型: 模型) -> 额外参数:
        ...
        if len(self.参数) == 0:
            ...
            return 额外参数之战场分析基站.尝试构造(self)
        elif 目标模型.序号 == 全息信标模型序号:
            return 额外参数之信标.尝试构造(self)
...
@dataclass
class 额外参数之信标(解析后额外参数):
    颜色: 类型.Int32  # 颜色索引
    投影高度: float  # 投影高度，存储时 ×100
    投影半径: float  # 投影半径，存储时 ×100
    ...
    def 去解析(self) -> 额外参数之未解析:
        参数列表 = [
            类型.Int32(int(self.颜色)),
            类型.Int32(int(round(self.投影高度 * 100))),
            类型.Int32(int(round(self.投影半径 * 100))),
            类型.Int32(int(self.标记等级)),
            类型.Int32(int(self.信息等级)),
            类型.Int32(int(self.图标)),
        ]
...
# 全息信标 模型序号 664（V0.10.34 新增，序号字典未收录时用数字）
全息信标模型序号 = 664
