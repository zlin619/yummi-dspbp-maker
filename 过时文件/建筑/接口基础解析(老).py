...
def 解析单个分拣器接口(该分拣器, 所有建筑):
    该分拣器["一级接口解析"] = {}
    该分拣器["一级接口解析"]["本建筑类型"] = "分拣器"
    # outputObjIdx = 该分拣器向xx目标输出
    该分拣器["一级接口解析"]["输出目标ID"] = 该分拣器['outputObjIdx']
     # inputObjIdx = 该分拣器从xx目标输入
    该分拣器["一级接口解析"]["输入目标ID"] = 该分拣器['inputObjIdx']
    # 传送带不记录上个传送带，只记录下个
...
def 解析单个普通建筑接口(该建筑, 所有建筑):
    ...
    该建筑["一级接口解析"] = {}
    该建筑["一级接口解析"]["本建筑类型"] = "一般建筑"
    该建筑["一级接口解析"]["输出目标ID"] = 该建筑['outputObjIdx']
    该建筑["一级接口解析"]["堆叠目标ID"] = 该建筑['inputObjIdx']
...
def 解析单个建筑接口(建筑, 所有建筑):
    if 建筑通用.是传送带吗(建筑):
        return 解析单个传送带接口(建筑, 所有建筑)
    elif 建筑通用.是分拣器吗(建筑):
        return 解析单个分拣器接口(建筑, 所有建筑)
    else:
        return 解析单个普通建筑接口(建筑, 所有建筑)
